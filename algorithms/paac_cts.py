...
class PAACCTSLearner(ActorLearner):
    ...
    def __init__(self, network_creator, environment_creator, args): 
        ...
        self._cts_density_model = CTSDensityModel(**model_args)
    ...
    def choose_next_actions(network, num_actions, states, session):
        ...
        action_indices = PAACCTSLearner.__sample_policy_action(network_output_pi)
    ...
    def __choose_next_actions(self, states):
        return PAACCTSLearner.choose_next_actions(self.network, self.num_actions, states, self.session)
    ...
    def rescale_reward(self, reward, state):
        ...
        if self.has_exploration_bonus:
            b = self.get_exploration_bonus(state)
            reward += b
        return reward


    def get_exploration_bonus(self, state):
        return self._cts_density_model.update(state)


    def cleanup(self):
        super(PAACCTSLearner, self).cleanup()
        self.runners.stop()
        ...
        while self.global_step < self.max_global_steps:
            ...
            for t in range(max_local_steps):
                ...
                for e, (actual_reward, episode_over) in enumerate(zip(shared_rewards, shared_episode_over)):
                    ...
                    actual_reward = self.rescale_reward(actual_reward, states[t])
